...
from dds_loader.load_currency_rates import load_dds_currency_rates
from dds_loader.load_transactions  import load_dds_transactions
...
def stg_to_dds():
    ...
    def load_dds_currency_rates_task(process_date: str) -> None:
        load_dds_currency_rates(
            process_date=datetime.fromisoformat(process_date).date()
        )

    process_date = "{{ data_interval_start | ds }}"

    load_transactions_task(process_date)
    load_currency_rates_task(process_date)
